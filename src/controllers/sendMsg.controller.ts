...
import { sendMsgServiceV2, sendMsgServiceV3 } from '../utils/service/lineService';
...
export const sendMsgController = {
  ...
  sendMsgByCard: handleErrorAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { company, dept, job, empId, message, requireConfirmation, channelId, groupCode } = req.body;
    // 依據條件取得目標成員資料
